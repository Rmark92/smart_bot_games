# Tictactoe with bot that makes move based on minimax calculation -- unbeatable
...
module Minimax
  ...
  def minimax(board, turn)
    if board.someone_won? || board.full?
      ...
      when @marker        then return 1
      when @other_marker  then return -1
      else                     return 0
      end
    end

    scores =  board.all_unmarked_keys.each_with_object(Hash.new) do |move, scores|
                new_board = board.copy
                new_board[move] = turn
                scores[move] = minimax(new_board, switch_turns(turn))
              end

    evaluate(scores, turn)
  end

  def evaluate(scores, turn)
    if turn == @marker
      @best_move, best_score = scores.max_by { |_, score| score }
    else
      @best_move, best_score = scores.min_by { |_, score| score }
    end
    best_score
  end

  def switch_turns(turn)
    turn == @marker ? @other_marker: @marker
  end
end
